'''Functions to test and train multiple enviroments.
...
from utils import trading_chart
...
def train_multiprocessing(CustomEnv, agent, train_df, num_worker=4, training_batch_size=500, visualize=False, EPISODES=10000):
    ...
    best_average = -10000  # used to track best average net worth

    for idx in range(num_worker):
        parent_conn, child_conn = Pipe()
        env = CustomEnv  # removing the redundant step
    ...
    while episode < EPISODES:
        predictions_list = agent.Actor.actor_predict(
            np.reshape(state, [num_worker]+[_ for _ in np.shape(state[0])]))  # fixing state[0].shape
        ...
        for worker_id, parent_conn in enumerate(parent_conns):
            ...
            if reset:
                ...
                print("episode: {:<5} worker: {:<2} net worth: {:<7.2f} reward: {:<7.2f} orders: {}".format(
                    episode, worker_id, net_worth, average, episode_orders))

                if EPISODES > len(total_average):
